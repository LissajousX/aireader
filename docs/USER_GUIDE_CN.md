...
- **顶部栏** — 侧边栏开关（☰）、文档标题、主题切换（🌓）、AI 面板开关（🤖）
- **侧边栏** — 导入文档按钮、文档列表、底部：文档库 / 设置
- **阅读区域** — 文档内容展示，支持目录侧栏、文本选择
- **AI 面板** — 翻译 / 释义 / 对话 / 笔记四个 Tab，支持模型切换和深度思考
- **浮动工具栏** — 目录、翻页、缩放、页码、阅读模式、文档主题
...
**Q: Linux 虚拟机中打开后界面全白？**
这是 WebKitGTK GPU 合成在虚拟机中渲染失败导致的。v1.1.0 未来版本应用会自动检测虚拟机环境并禁用 GPU 合成。如果仍遇到白屏，可手动设置环境变量启动：`WEBKIT_DISABLE_COMPOSITING_MODE=1 aireader`
